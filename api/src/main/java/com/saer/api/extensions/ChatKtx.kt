...
//    ) = api.getChatEventLog(this.id, query, fromEventId, limit, filters, userIds)
     ...
     */
    suspend fun Chat.getMember(userId: MessageSender) = api.getChatMember(this.id, userId)
...
//        api.getPublicMessageLink(this.id, messageId, forAlbum)
     ...
     */
    suspend fun Chat.openMessageContent(messageId: Long) =
        api.openMessageContent(this.id, messageId)
...
//        api.pinChatMessage(this.id, messageId, disableNotification)
     ...
     * @return [TdApi.Messages] Contains a list of messages.
     */
    suspend fun Chat.resendMessages(messageIds: LongArray?) =
        api.resendMessages(this.id, messageIds)
...
//        api.saveApplicationLogEvent(type, this.id, data)
     ...
     */
    suspend fun Chat.sendAction(action: ChatAction?, messageThreadId: Long) =
...
//        hideViaBot)
    ...
    suspend fun Chat.sendMessage(
        ...
        replyMarkup: ReplyMarkup?,
        inputMessageContent: InputMessageContent?
    ) = api.sendMessage(this.id, messageThreadId, replyToMessageId, options,
...
//    suspend fun Chat.setList(chatList: ChatList?) = api.setChatChatList(this.id, chatList)
     ...
     */
    suspend fun Chat.setDiscussionGroup(discussionChatId: Long) =
        api.setChatDiscussionGroup(this.id,
            discussionChatId)
...
//    ) = api.setGameScore(this.id, messageId, editMessage, userId, score, force)
     ...
     */
//    suspend fun Chat.setPollAnswer(messageId: Long, optionIds: IntArray?) =
//        api.setPollAnswer(this.id,
//            messageId, optionIds)
